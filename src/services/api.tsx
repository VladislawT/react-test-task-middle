import { CategoriesInterface, ProductsIterface, SizeInterface } from "@/types";
...
const products: ProductsIterface[] = [
    {
        id: 1,
        name: "Футболка",
        categoryId: 1,
...
function getProductColor(productID: number, colorID: number) {
    return new Promise((resolve, reject) => {
        setTimeout(() => {
            ...
            const color = product.colors.find((color) => String(color.id) === String(colorID));
...
export { getSizes, getSize, getCategories, getCategory, getProducts, getProduct, getProductColor };
